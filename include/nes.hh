...
public:
    ...
    //? File I/O
    //? ????????????????????????????????????????????????????????????????????????????????????
    u8 iNES_Read(const char* file_in); //TODO
    u8 iNES_Read(const char* file_in, uint16_t offset); //TODO
    u8 iNES_Read(const char* file_in, uint16_t offset, uint16_t width); //TODO
    u8 iNES_Write(const char* file_out,  vector<uint16_t> data, bool overwrite); //TODO
};
...
public:
    ...
    u8 SaveData(ifstream& iNES_in);
    u8 ExportData(ofstream& iNES_out);
